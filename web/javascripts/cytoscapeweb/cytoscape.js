$.widget("rbbt.cytoscape_agent", {
    ...
    _update_events: function(){
        ...
        var agent = this;
        vis.ready(function(){
            agent._process_aesthetics();
            if (agent.options.init == false){
                vis.removeListener("click", "nodes")
                vis.removeListener("click", "edges")
                vis.addListener("click", "nodes", function(event) {
                    agent.options.node_click(event);
                    return false;
                })
                .addListener("click", "edges", function(event) {
                    agent.options.edge_click(event);
                    return false;
                })
                agent.options.init = true;
            }
    ...
    _create: function() {
        this.element.addClass('cytoscape_agent_init')
    ...
    draw: function(){
        ...
        if (this.options.network === undefined){
            this._get_network(this.options.databases, function(network){
                agent.options.init = false
                agent.options.network = network
    ...
    add_neighbours: function(database){
        ...
        this._get_neighbours(database, this.options.entities, function(info){
          for (type in info){
            ...
            agent.add_entities(type, entities)
          }
          this.options.network = undefined;
          this.draw()
        })
    ...
    _map_continuous: function(elem_type, aesthetic, map, feature){
        ...
        if (undefined === map || null === map){
            ...
            $.each(elems, function(){
                var elem = this;
                var id = elem.data.id;
                var val = agent._elem_feature(elem, feature);
                map[id] = val;
            })
        }

        var elem_codes = $.map(elems, function(elem){return(agent._elem_feature(elem, feature))});
