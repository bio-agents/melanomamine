/*
 * jQuery Foundation Agenttips 2.0.2
...
;(function ($, window, undefined) {
  ...
  var settings = {
      ...
      agenttipClass : '.agenttip',
      tipTemplate : function (selector, content) {
        return '<span data-selector="' + selector + '" class="' + settings.agenttipClass.substring(1) + '">' + content + '<span class="nub"></span></span>';
      }
    },
    methods = {
      init : function (options) {
        ...
        return this.each(function () {
          ...
          if (Modernizr.touch) {
            $body.on('click.agenttip touchstart.agenttip touchend.agenttip', settings.selector, function (e) {
              e.preventDefault();
              $(settings.agenttipClass).hide();
              methods.showOrCreateTip($(this));
            });
            $body.on('click.agenttip touchstart.agenttip touchend.agenttip', settings.agenttipClass, function (e) {
              e.preventDefault();
              $(this).fadeOut(150);
            });
          } else {
            $body.on('mouseenter.agenttip mouseleave.agenttip', settings.selector, function (e) {
              var $this = $(this);
      ...
      getTip : function ($target) {
        ...
        if (selector) {
          tip = $('span[data-selector=' + selector + ']' + settings.agenttipClass);
        }
      ...
      selector : function ($target) {
        ...
        if (id === undefined && dataSelector === undefined) {
          dataSelector = 'agenttip' + Math.random().toString(36).substring(7);
      ...
      reload : function () {
        ...
        return ($self.data('agenttips')) ? $self.foundationAgenttips('destroy').foundationAgenttips('init') : $self.foundationAgenttips('init');
      },
      destroy : function () {
        return this.each(function () {
          $(window).off('.agenttip');
          $(settings.selector).off('.agenttip');
          $(settings.agenttipClass).each(function (i) {
  ...
  $.fn.foundationAgenttips = function (method) {
    if (methods[method]) {
      return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
    } else if (typeof method === 'object' || !method) {
      return methods.init.apply(this, arguments);
    } else {
      $.error('Method ' +  method + ' does not exist on jQuery.foundationAgenttips');
    }
  };
}(jQuery, this));
