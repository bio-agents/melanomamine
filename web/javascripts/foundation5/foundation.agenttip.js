  ...
  Foundation.libs.agenttip = {
    ...
    settings : {
      ...
      tip_template : function (selector, content) {
        return '<span data-selector="' + selector + '" class="' 
          + Foundation.libs.agenttip.settings.agenttip_class.substring(1) 
    ...
    events : function () {
      ...
      if (Modernizr.touch) {
        S(document)
          .off('.agenttip')
          .on('click.fndtn.agenttip touchstart.fndtn.agenttip touchend.fndtn.agenttip', 
            '[' + this.attr_name() + ']:not(a)', function (e) {
            var settings = $.extend({}, self.settings, self.data_options(S(this)));
            if (!settings.disable_for_touch) {
              e.preventDefault();
              S(settings.agenttip_class).hide();
              self.showOrCreateTip(S(this));
            }
          })
          .on('click.fndtn.agenttip touchstart.fndtn.agenttip touchend.fndtn.agenttip', 
            this.settings.agenttip_class, function (e) {
            e.preventDefault();
            S(this).fadeOut(150);
          });
      } else {
        S(document)
          .off('.agenttip')
          .on('mouseenter.fndtn.agenttip mouseleave.fndtn.agenttip', 
    ...
    getTip : function ($target) {
      ...
      if (selector) {
        tip = this.S('span[data-selector="' + selector + '"]' + this.settings.agenttip_class);
      }
    ...
    selector : function ($target) {
      ...
      if ((id && id.length < 1 || !id) && typeof dataSelector != 'string') {
        dataSelector = 'agenttip' + this.random_str(6);
    ...
    reload : function () {
      ...
      return ($self.data('fndtn-agenttips')) ? $self.foundationAgenttips('destroy').foundationAgenttips('init') : $self.foundationAgenttips('init');
    },

    off : function () {
      this.S(this.scope).off('.fndtn.agenttip');
      this.S(this.settings.agenttip_class).each(function (i) {
